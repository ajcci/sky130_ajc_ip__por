...
.save v(por)
"}
C {devices/code_shown.sym} -1200 -400 0 0 {name=CONTROL only_toplevel=false value=".csparam dvdd2=[\{Vdvdd\}/2]
.control
tran 100u 150m

meas tran por_start when v(por)=$&dvdd2 td=1.5m rise=1
meas tran por_end when v(por)=$&dvdd2 td=1.5m fall=1
let reset_active_time = $&por_end - $&por_start
echo $&reset_active_time

echo $&reset_active_time > \{simpath\}/\{filename\}_\{N\}.data
